...
export default function AuthPage() {
  ...
  const { user, loginMutation, pending2FAUserId, setPending2FAUserId, show2FAPrompt, setShow2FAPrompt } = useAuth();
  ...
  async function handle2FASubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setTwofaCode("");
      // Refetch user session and reload page to update state
      await fetch("/api/user", { credentials: "include" });
      window.location.href = "/";
    } catch (err: any) {
      toast({ title: "2FA Failed", description: err.message || "Invalid 2FA code", variant: "destructive" });
      setTwofaCode("");
  ...
  return (
    <>
      {/* 2FA Modal */}
      <Dialog open={show2FAPrompt} onOpenChange={setShow2FAPrompt}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Two-Factor Authentication</DialogTitle>
          </DialogHeader>
          <form onSubmit={handle2FASubmit} className="space-y-4">
            <label htmlFor="twofa-code" className="block text-sm font-medium text-gray-700">Enter your 2FA code</label>
            <input
              id="twofa-code"
              ref={twofaInputRef}
              type="text"
            ...
            <div className="flex justify-end space-x-2">
              <Button type="button" variant="secondary" onClick={() => { setShow2FAPrompt(false); setPending2FAUserId(null); setTwofaCode(""); }} disabled={is2FALoading}>Cancel</Button>
              <Button type="submit" disabled={is2FALoading || !twofaCode}>{is2FALoading ? "Verifying..." : "Verify"}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
