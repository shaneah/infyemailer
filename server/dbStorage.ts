...
export class DbStorage implements IStorage {
  ...
  async getClientUserPreferences(clientUserId: number): Promise<any> {
    try {
      const [clientUser] = await db.select().from(schema.clientUsers)
        .where(eq(schema.clientUsers.id, clientUserId));
      ...
      console.error("Error getting client user preferences:", error);
      return null;
    }
  }
  ...
  async updateClientUserPreferences(clientUserId: number, preferences: any): Promise<boolean> {
    try {
      ...
      await db.update(schema.clientUsers)
        .set({ metadata })
        .where(eq(schema.clientUsers.id, clientUserId));
        
      return true;
    } catch (error) {
      console.error("Error updating client user preferences:", error);
      return false;
    }
  }
